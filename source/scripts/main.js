...
const fieldSelect = {
	...
	toggleOptions: function(obj){
		var t = this, p = $(obj).closest(t.field);
...
const FormVal = {
	...
	init: function(){
		...
		$(t.el + t.elPassword + ' input').on('keyup', function(e) {
			$(this).closest(t.elContainer).find(t.el + t.elPasswordRepeat + ' input').val('');
			//t.validation();
		});
...
const gCounter = {
   ...
   shake: function() {
	...
	counterMinus: function(obj){
		let t = this, field = $(obj).closest(t.el).find(t.elInput)[0], i = 0;
	...
	buttons: function(obj){
		let t = this, parent = $(obj).closest(t.el);
		if(obj.value<=1){
	...
	calc: function(){
		var t = this, sum = 0, oldsum = 0, disc = 0;
...
const gProfile = {
	...
	openOrder: function(obj){
		var t = this, p = $(obj).closest(t.elOrders);
	...
	openBlock: function(obj){
		...
		let idx = p.find(t.elMenu).index($(obj));
	...
	init: function() {
		...
		if(t.sliderWrap.length > 0) {
			...
			$(t.sliderWrap).each(function() {
				let minPriceInput = $(this).find('[data-min]');
				let maxPriceInput = $(this).find('[data-max]');
